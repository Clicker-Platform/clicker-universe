...
export function BookingDetailPanel({ booking, onClose, onStatusUpdate, onUpdateDetails }: BookingDetailPanelProps) {
    ...
    if (!booking) {
        return (
            <div className="h-full flex flex-col items-center justify-center text-gray-300 p-12 text-center bg-gray-50/50">
                <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mb-6">
                    <Clock size={40} className="text-gray-300" />
                </div>
                <p className="font-bold text-xl text-gray-400 mb-2">No Booking Selected</p>
                <p className="text-gray-400 max-w-xs">Select a booking from the list on the left to view details and manage the reservation.</p>
            </div>
        );
    }
        ...
        <div className="animate-in fade-in duration-300 h-full flex flex-col">
            ...
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-10">
                <div>
                    ...
                    <div className="flex items-center gap-2">
                        <p className="text-xs text-gray-400 font-mono uppercase tracking-wider">ID: {booking.id.slice(0, 8)}</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    {booking.status !== 'cancelled' && !isEditing && (
                        <button
                            onClick={startEdit}
                            className="px-4 py-2 bg-gray-100 text-brand-dark rounded-lg text-sm font-bold hover:bg-gray-200 transition-colors"
                        >
                            Edit
                        </button>
                    )}
            ...
            <div className="p-8 overflow-y-auto flex-1">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    {/* Customer Info */}
                    <div>
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-3">Customer Info</label>
                    ...
                    {/* Service Details */}
                    <div>
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-3">Service Details</label>
                ...
                {isEditing ? (
                    <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 animate-in fade-in zoom-in-95 duration-200 mb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                                <label className="text-xs font-bold text-gray-500 uppercase block mb-1">Assigned Staff</label>
                                <input
                                    type="text"
                    ...
                    <div className="mb-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-3">Assigned Staff</label>
                                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                                    <div className="w-8 h-8 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-xs">
                                        {(booking.staffName || 'A').charAt(0)}
                                    </div>
                                    <p className="font-bold text-gray-900">{booking.staffName || 'Any Available'}</p>
                                </div>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-3">Additional Notes</label>
                ...
                {isMissingEmail && (
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-brand-dark/20 focus:border-brand-dark outline-none transition-all"
                            value={enrollEmail}
