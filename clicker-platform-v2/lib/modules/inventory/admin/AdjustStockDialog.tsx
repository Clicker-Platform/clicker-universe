        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                    <h3 className="font-bold text-lg text-gray-800">Adjust Stock: {item.name}</h3>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    <div className="p-5 bg-gray-50 rounded-xl text-center border border-gray-100">
                        <span className="text-gray-500 text-xs uppercase font-bold tracking-wider">Current Stock</span>
                        <div className="text-4xl font-black text-brand-dark mt-1">{item.currentStock}</div>
                        <div className="text-sm font-medium text-gray-400 mt-1">{item.unit}</div>
                    </div>

                    <div className="space-y-1.5">
                        <label className="block text-sm font-bold text-gray-700">Adjustment (+/-)</label>
                        <input
                            type="number"
                            required
                            className="w-full border border-gray-200 p-3 rounded-xl text-lg font-mono focus:ring-2 focus:ring-brand-dark/10 focus:border-brand-dark outline-none transition-all"
