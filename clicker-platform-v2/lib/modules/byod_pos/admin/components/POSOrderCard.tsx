...
export function POSOrderCard({ order, onUpdateStatus, onCancel, onProcessPayment, minimal, expanded, onClick, kds }: POSOrderCardProps) {
    ...
    const cardContent = (
        <div className={`overflow-hidden ${minimal
            ...
            : `bg-white rounded-3xl shadow-sm border-[3px] ${order.status === 'pending' ? 'border-brand-dark ring-0' : 'border-brand-dark'}`
            }`}>
            {/* Header */}
            <div
                className={`p-4 flex justify-between items-start transition-colors ${minimal
                    ? 'border-b border-gray-100 bg-gray-50'
                    : `border-b ${order.paymentStatus === 'pending_confirmation' ? 'bg-yellow-50 border-yellow-100' : 'bg-gray-50 border-gray-100'}`
                    }`}
                onClick={!minimal ? onClick : undefined}
                style={{ cursor: !minimal ? 'pointer' : 'default' }}
            >
                <div>
                    <h3 className="font-bold text-lg text-gray-900">#{order.id.slice(-4).toUpperCase()}</h3>
                ...
                <div className="flex flex-col items-end gap-2">
                    ...
                    {!minimal && (
                        <div className="flex items-center gap-1.5 text-xs font-semibold text-gray-600 bg-white px-2 py-1 rounded border border-gray-100">
            ...
            {!minimal && (
                <div className="px-4 py-2 bg-white border-b border-gray-100 flex items-center justify-between gap-4">
                ...
                <div className="p-4 space-y-3">
                        ...
                        <div key={idx} className="flex justify-between items-start">
                            <div className="flex gap-2">
                                ...
                                <div>
                                    ...
                                    {item.variantName && <span className="text-sm text-gray-500 block">({item.variantName})</span>}
                    ...
                    {!kds && (
                        <div className="pt-3 mt-3 border-t border-dashed border-gray-200">
                            ...
                            <div className="flex justify-between items-center font-bold text-lg mt-2 pt-2 border-t border-gray-100">
                                <span>Total</span>
            ...
            {!minimal && (
                <div className="p-3 bg-gray-50 border-t border-gray-100 grid grid-cols-2 gap-2">
                    ...
                    {order.status === 'completed' && (
                        <div className="col-span-2 text-center text-gray-400 text-sm font-medium py-2">
                            Completed
                        </div>
                    )}
                    ...
                    {order.status !== 'completed' && (
                        <button
                            ...
                            className="col-span-2 mt-2 bg-red-50 text-red-500 py-2 rounded-lg font-bold text-xs hover:bg-red-100 flex items-center justify-center gap-1 transition-colors active:scale-95"
                        >
