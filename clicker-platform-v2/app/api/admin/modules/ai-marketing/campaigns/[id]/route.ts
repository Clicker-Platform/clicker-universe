...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const doc = await adminDb.doc(`sites/${session.siteId}/${COLLECTION_CAMPAIGNS}/${params.id}`).get();
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await adminDb.doc(`sites/${session.siteId}/${COLLECTION_CAMPAIGNS}/${params.id}`).update(updates);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await adminDb.doc(`sites/${session.siteId}/${COLLECTION_CAMPAIGNS}/${params.id}`).delete();
  return NextResponse.json({ ok: true });
}
