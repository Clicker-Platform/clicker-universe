...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const doc = await adminDb.doc(`sites/${auth.session.siteId}/${COLLECTION_CAMPAIGNS}/${id}`).get();
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await adminDb.doc(`sites/${auth.session.siteId}/${COLLECTION_CAMPAIGNS}/${id}`).update(updates);
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  await adminDb.doc(`sites/${auth.session.siteId}/${COLLECTION_CAMPAIGNS}/${id}`).delete();
  return NextResponse.json({ ok: true });
}
