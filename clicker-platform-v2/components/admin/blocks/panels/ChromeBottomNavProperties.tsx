...
import { Navigation, GripVertical, Trash2, ChevronDown, ChevronUp, Link as LinkIcon, Plus, Check, AlertCircle, Loader2 } from 'lucide-react';
...
function FabIconPicker({ currentIcon, onSelect, onOpenIconPicker }: { currentIcon: string; onSelect: (icon: string) => void; onOpenIconPicker: (currentIcon: string, onSelect: (icon: string) => void) => void }) {
    ...
    return (
        ...
        >
            ...
            <span className="text-sm text-neutral-300">{currentIcon || 'Select Icon'}</span>
            <span className="ml-auto text-[10px] text-neutral-500">Change</span>
        </button>
    );
}
...
export function ChromeBottomNavProperties() {
    ...
    const [navigation, setNavigation] = useState<any>({ bottomNav: [], fab: null });
        ...
        <div className="space-y-5 animate-fade-in">
            ...
            <div className="border-t border-neutral-800 pt-4 space-y-3">
                <div className="flex items-center justify-between">
                    <div>
                        <h5 className="text-xs font-bold text-neutral-300 uppercase tracking-wider">Center FAB Button</h5>
                        <p className="text-[10px] text-neutral-600 mt-0.5">Floating action button in center of bottom bar</p>
                    </div>
                    <label className="flex items-center gap-2 cursor-pointer">
                        <span className="text-xs text-neutral-500">{fab?.enabled ? 'On' : 'Off'}</span>
                        <div className={`relative inline-flex h-5 w-9 rounded-full border-2 border-transparent transition-colors ${fab?.enabled ? 'bg-blue-500' : 'bg-neutral-700'}`}>
                            <input
                                type="checkbox"
                                className="sr-only"
                                checked={!!fab?.enabled}
                ...
                {fab?.enabled && (
                    <div className="space-y-3 animate-fade-in">
                        ...
                        <div>
                            <label className="block text-[10px] font-bold text-neutral-500 uppercase tracking-wider mb-1">Destination</label>
                            {fab?.type === 'form' ? (
                                <select
                                    value={fab?.formId || ''}
